...
export async function POST(
  ...
  try {
    ...
    const result = await castVote(pb, user.id, "post", id, value);
    ...
    if (value === 1 && result.userVote === 1) {
      try {
        ...
        // Award receive_post_upvote points to the post author
        const post = await pb.collection("posts").getOne(id);
        const authorId = post["author"] as string;
        if (authorId && authorId !== user.id) {
          await awardPoints(pb, authorId, "receive_post_upvote", "post", id);
        }
      } catch {
        // Non-critical
      }
